Substitua o seu server.js inteiro por este:
...
app.get('/api/state/:day', async (req, res) => {
  try {
    ...
    const surgeries = await all(`
      SELECT
        s.*,
        a.name AS anesthetist
      FROM surgeries s
      LEFT JOIN anesthetists a
        ON a.id = s.anesthetist_id
      WHERE s.day = ?
      ORDER BY s.room, s.start, s.id
    `, [day]);
    const anesthetists = await all(`
      SELECT *
      FROM anesthetists
      ORDER BY name
    `);
...
app.post('/api/surgeries', async (req, res) => {
  try {
    ...
    `, [
      safeDay(s.day),
      s.start || '07:00',
      s.room || 'Cirurgias não escaladas',
      s.title || 'Cirurgia',
      Number(s.duration) || 60,
      s.service || '',
      s.initials || '',
      s.age || '',
      s.obs || '',
      s.anesthetist_id || null
    ]);
    res.json({
      ok: true,
      id: result.id
...
app.post('/api/anesthetists', async (req, res) => {
  try {
    ...
    const result = await run(`
      INSERT OR IGNORE INTO anesthetists(name)
      VALUES(?)
    `, [name]);
    res.json({
      ok: true,
